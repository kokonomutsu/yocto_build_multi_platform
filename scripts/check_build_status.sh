...
for platform in s905x3 rpi4 rpi0w; do
    if [ -d "/home/yocto/logs" ]; then
        LATEST_LOG=$(ls -t /home/yocto/logs/build-${platform}-*.log 2>/dev/null | head -1)
        if [ -n "$LATEST_LOG" ]; then
            ...
            elif grep -q "ERROR\|Failed" "$LATEST_LOG" 2>/dev/null | tail -1; then
                echo "    ❌ Build failed"
    ...
    if [ -d "$BUILD_DIR" ]; then
        ...
        if [ -d "$BUILD_DIR/tmp/deploy/images" ]; then
            ...
            if [ "$IMAGES" -gt 0 ]; then
                ...
                find "$BUILD_DIR/tmp/deploy/images" -type f \( -name "*.wic" -o -name "*.ext3" -o -name "*.tar.bz2" -o -name "*.img" \) 2>/dev/null | head -2 | sed 's/^/    /'
            else
